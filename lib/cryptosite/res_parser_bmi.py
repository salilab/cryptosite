...
def res_packing(pdb, res_list, radius=4.):
    ...
    for r in res_list:
        ...
        ZZZ = []
        for a in res_list[r]:
            distances = numpy.sqrt(numpy.sum((XYZ - numpy.array(a)) ** 2,
                                   axis=1))
...
def distance_from_surface(all_atoms, surf_atoms):
    ...
    for a in range(numpy.shape(Dist)[1]):
        for s in range(numpy.shape(Dist)[0]):
            if Ida[a] not in Dist2Surf:
                Dist2Surf[Ida[a]] = Dist[s, a]
            else:
                Dist2Surf[Ida[a]] = min([Dist2Surf[Ida[a]], Dist[s, a]])
    return Dist2Surf
...
def charge_density(pdb, res_list, radius=5.):
    ...
    charge = {
        ...
        'GLY': 0, 'HIS': 0.5, 'ILE': 0, 'LEU': 0, 'LYS': 1, 'MET': 0,
    ...
    for d in D:
        if 'ATOM' == d[:4]:
            if d[17:20] != 'HOH':
                aid, res, rsid, chainid = (d[13:17], d[17:20],
                                           int(d[22:26]), d[21])
                x, y, z = float(d[30:38]), float(d[38:46]), float(d[46:54])
    ...
    for d in D:
        if 'ATOM' == d[:4]:
            if d[17:20] != 'HOH':
                aid, res, rsid, chainid = (d[13:17], d[17:20], int(d[22:26]),
                                           d[21])
                if float(d[60:66]) > 2.:
                    SurfRes[(res, rsid, chainid)] = 0
    for d in D:
        if 'ATOM' == d[:4]:
            if d[17:20] != 'HOH':
                aid, res, rsid, chainid = (d[13:17], d[17:20], int(d[22:26]),
                                           d[21])
                if (res, rsid, chainid) in SurfRes:
                    SurfRes[(res, rsid, chainid)] += float(d[60:66])
    ...
    for r in res_list:
        contacts = {}
        for a in res_list[r]:
            distances = numpy.sqrt(numpy.sum((CRDS - numpy.array(a)) ** 2,
                                   axis=1))
...
def get_patchmap(apo):
    ...
    for d in D:
        ...
        if len(d) > 0:
            if 'X' in d[1]:
                res, resid, chainid = 'UNK', int(d[2]), d[3]
            else:
                res, resid, chainid = d[1], int(d[2]), d[3]
...
def res_parser(fil):
    ...
    Header += '\t'.join(['SAS', 'PRT', 'CVX', 'CNC', 'SSE', 'HYD', 'CHR',
                         'SQC', 'PCK', 'D2S', 'PTM'] +
                        ['NBG', 'SASn', 'PRTn',
                         'CVXn', 'CNCn', 'Un', 'Bn', 'En', 'Gn', 'Hn', 'Sn',
                         'Tn', 'In', 'HYDn', 'CHRn', 'SQCn', 'PCKn'])
    ...
    for chain in Apo:
        for a in Apo[chain]:
            ...
            if float(sas) >= 2.:
                if p not in APO['SurfAtoms']:
                    APO['SurfAtoms'][p] = []
                APO['SurfAtoms'][p].append((float(a[30:38]), float(a[38:46]),
                                            float(a[46:54])))

            if float(sas) >= -10.:
                if p not in APO['AllAtoms']:
                    APO['AllAtoms'][p] = []
                APO['AllAtoms'][p].append((float(a[30:38]), float(a[38:46]),
                                           float(a[46:54])))

            if p not in APO['SurfaceArea']:
                APO['SurfaceArea'][p] = [float(sas)]
            else:
                APO['SurfaceArea'][p].append(float(sas))

            if p not in APO['Protrusion']:
                APO['Protrusion'][p] = [float(prt)]
            else:
                APO['Protrusion'][p].append(float(prt))
            ...
            if p not in APO['Convexity']:
                ...
                if seqcons[chain][1] != 0.:
                    APO['SeqCon'][p] = ((float(sqc) - seqcons[chain][0])
                                        / seqcons[chain][1])
                else:
                    APO['SeqCon'][p] = 0.
    ...
    Hdr = ['SAS', 'PRT', 'CVX', 'CNC', 'SSE', 'HYD', 'CHR', 'SQC', 'PCK',
           'D2S', 'PTM']
    Hdr += ['NBG', 'SASn', 'PRTn', 'CVXn', 'CNCn', 'Un', 'Bn', 'En', 'Gn',
            'Hn', 'Sn', 'Tn', 'In', 'HYDn', 'CHRn', 'SQCn', 'PCKn']

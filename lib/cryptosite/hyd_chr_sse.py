...
'''
hydrophobicity = {'A': 0.17, 'R': 0.81, 'N': 0.42, 'D': 1.23, 'C': -0.24,
...
def HydChrSSE(pdb, chain):
    ...
    # --- added cid -> chainID, fields change in XXX.hcs. XXX.ssp contains
    # all chains but DIC only the ones from query - second if
    # clausel "cid in chains".
    ...
    for d in D:
        ...
        if cid == chain:
            try:
                L = '\t'.join([rid, res, ss, str(hydrophobicity[res]),
                               str(charge[res])]) + '\n'
            except KeyError:
                onelet = PDB.Polypeptide.three_to_one(DIC[str(int(rid)) + cid])
                L = '\t'.join([rid, res, ss, str(hydrophobicity[onelet]),
                               str(charge[onelet])]) + '\n'
            out.write(L)
    out.close()

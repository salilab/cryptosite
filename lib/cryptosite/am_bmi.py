...
def protein_protrusion(pdb):
    ...
    for d in D:
        if 'ATOM' == d[:4]:
            ...
            xyz = numpy.array([float(d[30:38]), float(d[38:46]),
                               float(d[46:54])])
...
def protein_convexity(pdb):
    ...
    for d in D:
        if 'ATOM' == d[:4] and d[17:20] != 'HOH':
            x, y, z = float(d[30:38]), float(d[38:46]), float(d[46:54])
            atid, rsid, sas, cid = (int(d[6:11]), int(d[22:26]),
                                    float(d[60:66]), d[21])
            if cid == ' ':
                cid = 'A'
            if (rsid, cid) not in XYZ:
                XYZ[rsid, cid] = [[atid, x, y, z, sas]]
            else:
                XYZ[(rsid, cid)].append([atid, x, y, z, sas])
    ...
    for r1 in range(len(RX) - 1):
        F1 = SurfRes[RX[r1]]
        C1 = numpy.array([numpy.array([c[1], c[2], c[3]])
                          for c in F1 if c[4] > 2.])
        for r2 in range(r1 + 1, len(RX)):
            F2 = SurfRes[RX[r2]]
            C2 = numpy.array([numpy.array([c[1], c[2], c[3]])
                              for c in F2 if c[4] > 2.])
            ...
            if numpy.min(distance) < 6.:
                ...
                if a1 not in SurfNet:
                    SurfNet[a1] = [fc]
                else:
                    SurfNet[a1].append(fc)
                if a2 not in SurfNet:
                    SurfNet[a2] = [fc]
                else:
                    SurfNet[a2].append(fc)
...
def make_features():
    ...
    DRS = [i.strip().split()[0] for i in Data
           if float(i.strip().split()[-1]) < min(soap_scores) * .4]
    ...
    for dr in DRS:
        ...
        try:
            ...
            # replace with a more specific except block.
            raise
        ...
        for d in D:
            d = d.strip()
            if d[:4] == 'ATOM':
                atom, res, resid, cid = (d[12:16], d[17:20], int(d[22:26]),
                                         d[21])
                if cid == ' ':
                    cid = 'A'
                if atom == ' OXT':
        ...
        for p in RES:
            L = [str(i) for i in list(p) + [numpy.mean(RES[p]['sas14']),
                                            numpy.mean(RES[p]['sas30']),
                                            numpy.mean(RES[p]['prt']),
                                            numpy.mean(RES[p]['cvx'])]]
            out.write('\t'.join(L) + '\n')
    out.close()

    ...
    for d in D:
        if d[:4] == 'ATOM':
            ...
            for p in Pockets:
                ...
                if mini <= 5.:
                    adist[p] = mini
            _, res, resid, cid = d[12:16], d[17:20], int(d[22:26]), d[21]
...
def get_hcs(apo, achain):
    ...
    with open('%s%s.hcs' % (apo, achain)) as data:
        D = data.readlines()
...
def get_sqc(apo, achain):
    ...
    with open('%s%s.sqc' % (apo, achain)) as data:
        D = data.readlines()
    ...
    for chain in PDBChainOrder:
        ...
        for d in D:
            ...
            if d[:4] == 'ATOM' and (d[21] == chain):
                ...
                L = [d, sasai, prtai, cncai, cvxai] + hcsai \
                    + [sqcai] + list(cncfa)
                outf.write('\t'.join(L) + '\n')
        outf.close()

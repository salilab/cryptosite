...
def make_ligand_file(fname):
    ...
    with open(fname, 'w') as fh:
        fh.write('\n'.join(sorted(glob.glob(os.path.join(
            cryptosite.config.datadir, 'ligands', '*.pdb')))))
...
def transform(t, xyz):
    ...
    M = np.array([np.array([cz * cy, -sy * sx * cz - sz * cx,
                            -sy * cx * cz + sz * sx]),
                  np.array([sz * cy, -sy * sx * sz + cx * cz,
                            -sy * cx * sz - sx * cz]),
...
def patchmap_feature(pdb):
    ...
    for i, lig in enumerate(ligands):
        ...
        print(i, lig, pdb)
        data = open('mol2_score.res')
        LS = [float(ln.strip().split()[-1]) for ln in data.readlines()]
        data.close()
    ...
    for c in Counts:
        output.write('\t'.join([str(j) for j in c]) + '\t'
                     + str(Counts[c]) + '\n')
    output.close()

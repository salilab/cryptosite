...
def process_directory(tts):
    ...
    for pdb in pdbs:
        ...
        for e in efiles:
            ...
            # --- read in Qi
            try:
                with open(e.rsplit('/', 1)[0]
                          + '/qioft_pm_XXX.pdb_11sc.dat') as data:
                    D = data.readlines()
                q = []
                if len(D) > 0:
                    for d in D:
                        q.append(np.array([float(i)
            ...
            try:
                pocket_files = glob.glob(tts + '/pred_dECALCrAS1000/'
                                         + pdb + '/pockets.out')
                with open(pocket_files[0]) as data:
                    D = data.readlines()
                p = []
                Res = []
                ...
                for d in D[1:]:
                    ...
                    if len(d) > 5:
                        ...
                        if len(sd) == len_limit:
                            p.append(np.array(sd))
                        elif len(sd) < len_limit:
                            p.append(np.array(
                                sd + [0.] * (len_limit - len(sd))))
                ...
                pass
            # --- read in geometry
            files = glob.glob(tts + '/pred_dECALCrAS1000/' + pdb
                              + '/am_features.out')
            if len(files) != 1:
                continue
            for fil in files:
    ...
    H = ['Res', 'ResID', 'SAS14_mean_', 'SAS14_std_', 'SAS30_mean_',
         'SAS30_std_', 'PRT_mean_', 'PRT_std_']
    H += ['CVX_mean_', 'CVX_std_', 'QI_mean_', 'QI_std_', 'CNC_mean_',
          'CNC_std_']
    H += ['CN5_mean_', 'CN5_std_', 'CNS_']
    out.write('\t'.join(H) + '\n')
    for res in RES:
        L = [res[0], res[1], res[2], np.mean(AM['sas14'][res]),
             np.std(AM['sas14'][res])]
    ...
    out.close()
    print(np.shape(qm), np.shape(qd), np.shape(pm), np.shape(pd),
          np.shape(pm5), np.shape(pms), pdb)

    # --- create the final file with all the features

    Header = ['ApoID', 'Res', 'ResID', 'ChainID', 'SAS', 'PRT', 'CVX', 'CNC',
              'SSE', 'HYD', 'CHR', 'SQC', 'PCK', 'D2S', 'PTM', 'NBG', 'SASn',
              'PRTn', 'CVXn', 'CNCn', 'Un', 'Bn', 'En', 'Gn', 'Hn', 'Sn',
              'Tn', 'In', 'HYDn', 'CHRn', 'SQCn', 'PCKn', 'SAS14_mean_',
              'SAS14_std_', 'SAS30_mean_', 'SAS30_std_', 'PRT_mean_',
              'PRT_std_', 'CVX_mean_', 'CVX_std_', 'QI_mean_', 'QI_std_',
              'CNC_mean_', 'CNC_std_', 'CN5_mean_', 'CN5_std_', 'CNS_', 'CBS']

    files = glob.glob('*.am')
    pdbs = set([i.split('_')[0].split('/')[-1] for i in files])
    for a in pdbs:
        fff = a.split('.')[0]
        with open(fff + '_mdl.bmiftr') as data:
            D = data.readlines()
        ...
        with open('%s.am' % fff) as data:
            D = data.readlines()

        H += ['SAS14_mean_', 'SAS14_std_', 'SAS30_mean_', 'SAS30_std_',
              'PRT_mean_', 'PRT_std_']
        H += ['CVX_mean_', 'CVX_std_', 'QI_mean_', 'QI_std_', 'CNC_mean_',
              'CNC_std_']
        H += ['CN5_mean_', 'CN5_std_', 'CNS_']
